...
#### 4. Test impact of technical variability (measure the same sample more times) -------------------------------
...
for (ratio in ratios) {
  ...
  for (err in tecn_err){
    ...
    beta_value_victim <- beta_values_autosomal_cg_noNAs$AF_0 + random_errors
    ...
    if (ratio == "1:1" & err == 0) {
      table_pred_age_main_contr <- data.frame(ratio = character(),
                                              ...
                                              skinHorvath = numeric())
      table_pred_age_main_contr <- rbind(table_pred_age_main_contr, 
                                         c("Single_source", NA, predicted_age[predicted_age$Sample_name == "AE_0",
                                                                          c("Horvath", "BLUP", "EN", "skinHorvath")]))
...
table_pred_age_main_contr[, c(3,4,5,6)] <- abs(table_pred_age_main_contr[, c(3,4,5,6)] - 
  metadata_bloodstains[metadata_bloodstains$Sample_name == "AE_0","Age"])

if (i == 1 & ratio == "10:1" & err == 0.10) {
...
saveRDS(df, paste0(results_path, "Age_prediction_tecnology_err_10_simulations_18-07-2024.rds"))


df[, c(3,4,5,6)] <- df[, c(3,4,5,6)]/10
...
table_pred_age_main_contr_long$Tecnical_error <- as.numeric(table_pred_age_main_contr_long$Tecnical_error)

table_pred_age_main_contr_long <- table_pred_age_main_contr %>%
  pivot_longer(cols = -c(Ratio, Tecnical_error),
               names_to = "Clock", 
               values_to = "MAE")

str(table_pred_age_main_contr_long)
table_pred_age_main_contr_long$Tecnical_error <- as.numeric(table_pred_age_main_contr_long$Tecnical_error)

table_pred_age_main_contr_long$Tecnical_error[is.na(table_pred_age_main_contr_long$Tecnical_error)] <- 0
df_long$Tecnical_error[is.na(df_long$Tecnical_error)] <- 0
...
#### 5. Test impact of STR ratio calculation (measure the same sample more times) -------------------------------

n <- 123
for (i in c(1:10)){
ratios <- c("1:1", "1:4", "1:10", "4:1", "10:1")
#ratio <- "1:10"
STR_err <- c(-2, -1, -0.5, -0.2, 0, 0.2, 0.5, 1, 2) 
#err <- 1
set.seed(n) 
n <- n+1
random_signs <- sample(c(-1, 1), length(beta_values_autosomal_cg_noNAs$AF_0), replace = TRUE)
...
for (ratio in ratios) {
  ...
  for (err in STR_err){
    ...
    if (ratio_victim > ratio_offender ){
      ratio_victim_err <- ratio_victim + err
      ratio_offender_err <- ratio_offender
      
    } else {
      if (ratio == "1:1" & err < 0) {
        ratio_victim_err <- ratio_victim + abs(err)
        ratio_offender_err <- ratio_offender
      } else {
        ratio_offender_err <- ratio_offender + err
        ratio_victim_err <- ratio_victim
      }}
...
ggplot(df_long, aes(x = Tecnical_error, y = MAE, color = Ratio, group = interaction(Ratio, Clock))) +
  geom_line(size = 1) +  # Connect points with lines
  geom_point(size = 2) +  # Optional: Add points at each time point
  facet_wrap(~ Clock) +  # Facet by the "Group" variable
  theme_minimal() +  # Use a minimal theme
  labs(title = "",
       ...
       color = "Ratio") +
  coord_cartesian(ylim = c(0, 25)) +
  scale_x_continuous(breaks = c(-2, -1, -0.5, -0.2, 0, 0.2, 0.5, 1, 2),
  #                    #labels = c("0", "0.005", "0.01", "0.02", "0.03", "0.05", "0.1"),
...
#### 5. Test impact of STR ratio calculation (measure all combination of samples) -------------------------------

pairs <- combn(colnames(beta_values_autosomal_cg_noNAs)[grepl("0",colnames(beta_values_autosomal_cg_noNAs))],2)
pair_list <- apply(pairs, 2, function(x) paste(x[1], x[2], sep = ", "))
pair <- "AF_0, AD_0"
ratio <- "10:1"
err <- 2

  
for (pair in c("AF_0, AE_0", "AB_0, AA_0", "AD_0, AC_0", "AG_0, AC_0")){
  #ratios <- c("1:1", "1:4", "1:10", "4:1", "10:1")
  ...
  #err <- 0
  victim_name <- substr(pair, 1, 4) 
  offender_name <- substr(pair, nchar(pair) - 3, nchar(pair))
  
  #random_signs <- sample(c(-1, 1), length(beta_values_autosomal_cg_noNAs$AF_0), replace = TRUE)
  #random_errors <- random_signs * 0.01
  beta_value_victim <- beta_values_autosomal_cg_noNAs[[victim_name]]
  
  for (err in STR_err) {
    #Create mixture
    
    for (ratio in ratios){
      print(paste(pair, ratio, err))
      ratio_split <- strsplit(ratio, ":")
      ...
      if (ratio == "50:50" & err == -5) {
        table_pred_age_main_contr <- data.frame(ratio = character(),
                                                ...
                                                skinHorvath = numeric())
        table_pred_age_main_contr <- rbind(table_pred_age_main_contr, 
                                           c("Single_source", NA, predicted_age[predicted_age$Sample_name == offender_name,
                                                                                c("Horvath", "BLUP", "EN", "skinHorvath")]))
        colnames(table_pred_age_main_contr)[c(1,2)] <- c("Ratio", "Tecnical_error") 
      }
      mixture_28_55_ratio <- mixture_generator(beta_victim=beta_values_autosomal_cg_noNAs[[victim_name]],
                                               beta_offender_1=beta_values_autosomal_cg_noNAs[[offender_name]],
                                               ratio_victim = ratio_victim, ratio_offender = ratio_offender)
  ...
  table_pred_age_main_contr[, c(3,4,5,6)] <- abs(table_pred_age_main_contr[, c(3,4,5,6)] - 
                                                   metadata_bloodstains[metadata_bloodstains$Sample_name == offender_name,"Age"])
...
for (pair in c("AF_0, AE_0", "AB_0, AE_0", "AD_0, AE_0", "AG_0, AE_0")){
  ratios <- c("1:1", "1:4", "1:10", "4:1", "10:1")
  victim_name <- substr(pair, 1, 4) 
  offender_name <- substr(pair, nchar(pair) - 3, nchar(pair))
  ...
  random_signs <- sample(c(-1, 1), length(beta_values_autosomal_cg_noNAs$AF_0), replace = TRUE)
  random_errors <- random_signs * 0.01
  beta_value_victim <- beta_values_autosomal_cg_noNAs[[victim_name]] + random_errors
  #Create mixture
  for (ratio in ratios){
      print(paste(pair, ratio))
      ratio_split <- strsplit(ratio, ":")
      ratio_victim <- as.numeric(ratio_split[[1]][1])
      ratio_offender <- as.numeric(ratio_split[[1]][2])
      
      if(pair == "AF_0, AE_0" & ratio == "1:1"){
      ...
      predicted_age_mixture <- DNAmAge(df_for_prediction, clocks = c("BLUP", "EN", "Horvath", "skinHorvath"))
      colnames(predicted_age_mixture)[1] <- "Ratio"
      predicted_age_mixture$Pair <- pair
      predicted_age_mixture$Ratio <- ratio
      predicted_age_mixture$Age_gap <- round(abs(metadata_bloodstains[metadata_bloodstains$Sample_name == offender_name,"Age"] -
                                           metadata_bloodstains[metadata_bloodstains$Sample_name == victim_name,"Age"]), 2)
...
table_pred_age_main_contr$Ratio <- table_pred_age_main_contr$Ratio
...
df_long$Ratio <- factor(df_long$Ratio, levels = c("1:10", "1:4", "1:1", "4:1", "10:1"))

df_long$Age_gap <- sprintf("%.1f", df_long$Age_gap)

age_gap_plot <- ggplot(df_long, aes(x = Ratio, y = MAE, color = as.character(Age_gap), group = interaction(Age_gap, Clock))) +
  geom_line(size = 1) +  # Connect points with lines
  geom_point(size = 2) +  # Optional: Add points at each time point
  facet_wrap(~ Clock) +  # Facet by the "Group" variable
  theme_minimal() +  # Use a minimal theme
...
ggsave(paste0(results_path,"Plot_Age_difference_contributors_22-11-2024.png"), 
       age_gap_plot, width = 13, height = 8, dpi = 600, bg = "white")


#### 7. Test impact of Age differences in absolute error of offender from DNA mixture 2021 Longitudinal Cohort ---------------
...
n <- 1
for (n in 1:nrow(random_pairs)){
  ind_1 <- random_pairs[n,][1]
  ind_2 <-random_pairs[n,][2]
  print(paste(predicted_age_metadata_2021[predicted_age_metadata_2021$Sample_ID==ind_1, "AE"],
        predicted_age_metadata_2021[predicted_age_metadata_2021$Sample_ID==ind_2, "AE"]))
  print(abs(predicted_age_metadata_2021[predicted_age_metadata_2021$Sample_ID==ind_1, "Age"] -
            predicted_age_metadata_2021[predicted_age_metadata_2021$Sample_ID==ind_2, "Age"]))
}

print(abs(predicted_age_metadata_2021[predicted_age_metadata_2021$Sample_ID == "RHB112_2021", "Age"] -
            predicted_age_metadata_2021[predicted_age_metadata_2021$Sample_ID == "RHB105_2021", "Age"]))
...
for (n in 1:nrow(random_pairs)){
  ...
  for (ratio in ratios){
    ...
    if(victim_name == "RHB117_2021" & offender_name == "RHB069_2021" & ratio == "1:1"){
      table_pred_age_main_contr <- data.frame(Pair = character(),
                                              Ratio = character(),
                                              Age_gap = numeric(),
                                              Horvath = numeric(),
